...
class TowerSuite extends munit.FunSuite:
  ...
  test("Tower splitAmnt3 single-loop with IArray API") {
    // Test data
    val years = IArray(2024, 2024, 2024, 2025, 2025, 2026)
    val days = IArray(1, 100, 200, 50, 150, 75) 
    val losses = IArray(3.0, 8.0, 30.0, 15.5, 15.5, 30.0)
    ...
    // Run new single-loop implementation
    val (cededFlat3, retained3, splits3) = tower.splitAmnt3(years, days, losses)
    
    // Run reference implementation for comparison
    val (cededMatrix, retainedRef) = tower.splitAmnt(years.asInstanceOf[Array[Int]], days.asInstanceOf[Array[Int]], losses.asInstanceOf[Array[Double]])
    
    // Verify splits structure
    assertEquals(splits3.length, 2, "Should have 2 layer splits")
    assertEquals(splits3(0)._1, tower.layers(0), "First split should reference first layer")
    assertEquals(splits3(1)._1, tower.layers(1), "Second split should reference second layer")
    
    // Verify per-layer arrays
    for layerIdx <- 0 until 2 do
      val (layer, layerData) = splits3(layerIdx)
      assertEquals(layerData.length, losses.length, s"Layer $layerIdx data length")
    ...
    for layerIdx <- 0 until 2 do
      for i <- 0 until losses.length do
        val flatIdx = layerIdx * losses.length + i
        assertEqualsDouble(cededFlat3(flatIdx), cededMatrix(i, layerIdx), 1e-10, s"Flat ceded[$flatIdx]")
      end for
    end for
    
    // Verify retained matches reference
    assertEquals(retained3.length, retainedRef.length)
    for i <- retained3.indices do
      assertEqualsDouble(retained3(i), retainedRef(i), 1e-10, s"Retained[$i]")
    end for
    
    // Verify no leakage - total ceded + retained should equal total losses
    val totalCeded = cededFlat3.sum
    val totalRetained = retained3.sum
    val totalLosses = losses.asInstanceOf[Array[Double]].sum
    assertEqualsDouble(totalCeded + totalRetained, totalLosses, 0.001, "No leakage check")
    
    println("splitAmnt3 single-loop implementation matches reference exactly!")
    println(s"Splits layer 1: ${splits3(0)._2.mkString("[", ",", "]")}")
    println(s"Splits layer 2: ${splits3(1)._2.mkString("[", ",", "]")}")
    println(s"Retained: ${retained3.mkString("[", ",", "]")}")
  }
