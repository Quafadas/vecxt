...
object PlatformReporting:
  ...
  inline def lossReportFast(
    ...
    // Single pass through groups (similar to groupSum but computing all metrics inline)
    var i = 0
    var currentGroup = 1
    while currentGroup <= numIterations do
      var groupSum = 0.0

      // Sum all values in this group - use scalar loop since group sizes are typically small
