...
object PositionCalculations:
  /**
    ...
    * @param priceUnit the unit `price` is quoted in (e.g. `Pts` for a price of 102 meaning 102% of par)
    * @param priceDate the date on which `price` was observed; the forecast is for one year after this date
    * @param maturity the maturity date of the bond
    * @return the forecast price one year after `priceDate`, in `priceUnit`. If the bond matures within the year,
    *   this is par.
    */
  ...
  /**
    ...
    * @param priceDate the date on which `price` was observed; the projection is for one year after this date
    * @param maturity the maturity date of the bond
    * @return the forecast price change (forecast price minus `price`) in `priceUnit`. Positive for a bond priced
    *   below par, negative above par. If the bond matures within the year, this is the full distance to par.
    */
  def pull2Parity1Year(
    ...
    assert(maturity >= priceDate, s"Maturity $maturity and pricing date $priceDate suggest this bond has already matured. It should not pull to parity" )
    ...
    if (maturity < projectionDate)
      one - price
    else
      days2Project.toDouble / days2Maturity.toDouble * ( one - price)
