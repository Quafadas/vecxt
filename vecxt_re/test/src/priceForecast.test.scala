...
class PositionCalculationsSuite extends munit.FunSuite:
  ...
  test("Early maturity"):
    val reportDate = LocalDate.of(2026, 1, 1)
    val forecastPrice =
      PositionCalculations.priceForecast1Year(102, PriceUnit.Pts, reportDate, reportDate.plusMonths(6))
  ...
  test("refuses already matured"):
    ...
    intercept[java.lang.AssertionError](
      PositionCalculations.priceForecast1Year(102, PriceUnit.Pts, reportDate, reportDate.plusYears(-1))
    )

  test("Interpolation to a point"):
    val reportDate = LocalDate.of(2026, 1, 1)
    val forecastPrice =
      PositionCalculations.priceForecast1Year(10200, PriceUnit.Bps, reportDate, reportDate.plusYears(2))
