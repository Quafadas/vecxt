...
Enforcing this needs the compiled size, so it belongs to the dynamic tier: check D2 of [#105](https://github.com/Quafadas/vecxt/issues/105), which reads it from `LogCompilation` output.
...
The rule above is a convention. Three parts of it are now checked on every PR by `bytecodeAudit` ([#105](https://github.com/Quafadas/vecxt/issues/105)), which reads the budgets off the running JVM rather than trusting the numbers in this page — including the `HugeMethodLimit` caveat in the paragraph above, which the report labels as an assumed constant rather than a discovered one:
...
And `bytecodeAudit` reads bytecode, so none of C1/C2/C3 can see the `InlineSmallCode` limit described above. The two budgets those checks enforce are proxies for a machine-code constraint that is, for vectorised kernels, tighter than either of them. A passing `@HotPath` therefore means "inside the bytecode budget", not "C2 will inline this" — the stronger reading needs D2.
