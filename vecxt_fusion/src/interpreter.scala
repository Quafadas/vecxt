...
object Interpreter:
  ...
  private def unary(op: UnaryOp, a: IVal): IVal = a match
    case IVal.F64(nd) =>
      ...
      op match
        ...
        case UnaryOp.Sin =>
          while i < n do
            out(i) = math.sin(data(i)); i += 1
        case UnaryOp.Cos =>
          while i < n do
            out(i) = math.cos(data(i)); i += 1
        case UnaryOp.Tan =>
          while i < n do
            out(i) = math.tan(data(i)); i += 1
        case UnaryOp.Exp =>
          while i < n do
            out(i) = math.exp(data(i)); i += 1
        case UnaryOp.Log =>
          while i < n do
            out(i) = math.log(data(i)); i += 1
        case UnaryOp.Sqrt =>
          while i < n do
            out(i) = math.sqrt(data(i)); i += 1
        case UnaryOp.Abs =>
          while i < n do
            out(i) = math.abs(data(i)); i += 1
        case UnaryOp.Reciprocal =>
          while i < n do
            out(i) = 1.0 / data(i); i += 1
  ...
  private def castVal(to: DType, a: IVal): IVal = (a, to) match
    ...
    case (IVal.Bool(nd), DType.F64) =>
      ...
      var i = 0
      while i < d.length do
        out(i) = if d(i) then 1.0 else 0.0; i += 1
      end while
      IVal.f64cm(out, nd.shape.clone())
    case (IVal.Bool(nd), DType.I64) =>
      ...
      var i = 0
      while i < d.length do
        out(i) = if d(i) then 1L else 0L; i += 1
      end while
      IVal.i64cm(out, nd.shape.clone())
    case _ =>
  ...
  private def where(cond: IVal, x: IVal, y: IVal): IVal =
    ...
    (x, y) match
      case (IVal.F64(ndx), IVal.F64(ndy)) =>
        ...
        var i = 0
        while i < n do
          out(i) = if condData(i) then dx(i) else dy(i); i += 1
        end while
