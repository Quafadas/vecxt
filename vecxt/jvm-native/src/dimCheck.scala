...
protected[vecxt] object dimCheckLen:
  inline def apply[A](a: Array[A], b: Int) =
    if doCheck then if a.length != b then throw VectorDimensionMismatch(a.length, b)
end dimCheckLen

protected[vecxt] object dimCheck:
  inline def apply[A, B](a: Array[A], b: Array[B]) =
    if doCheck then if a.length != b.length then throw VectorDimensionMismatch(a.length, b.length)
