...
/** `*=` already had a real implementation to mirror for `-=`/`/=`, and each platform already had its own more
  * specialised `+=(n: Double)`, so only `d += m` needed wiring up to it - not a new `+=` implementation. Covers dense
  * contiguous (including a size that isn't a multiple of any common SIMD width, to catch tail-loop bugs like the one
  * fixed in `Array[Double]#-=(d: Double)` alongside this) and genuinely non-contiguous (padded-stride) layouts, plus
  * the `Double`-first spellings.
  */
