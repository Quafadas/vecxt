...
class ArrayAdditionSuite extends munit.FunSuite:
  ...
  test("array addition to columns (dim 0)"):
    ...
    val rowVector = NArray[Double](10.0, 20.0, 30.0) // Should be added to each column
    mat += rowVector

    assertMatrixEquals(
      mat,
      Matrix.fromRows(
        NArray[Double](11.0, 22.0, 33.0),
        NArray[Double](14.0, 25.0, 36.0),
        NArray[Double](17.0, 28.0, 39.0)
      )
    )
  ...
  test("array addition with dimension mismatch should throw exception"):
    ...
    val wrongSizeArray = NArray[Double](1.0, 2.0) // Only 2 elements, but matrix has 3 columns
  ...
  test("array addition to single row matrix"):
    ...
    val arr = NArray[Double](100.0, 200.0, 300.0)
    mat += arr

    assertMatrixEquals(
      mat,
      Matrix.fromRows(
        NArray[Double](101.0, 202.0, 303.0)
      )
    )
  ...
  test("SIMD-optimized array addition to columns (dense column-major)"):
    ...
    val mat = Matrix.fromRows[Double](
      ...
      NArray[Double](9.0, 10.0, 11.0, 12.0),
      NArray[Double](13.0, 14.0, 15.0, 16.0),
      NArray[Double](13.0, 14.0, 15.0, 16.0)
    )
    ...
    assertMatrixEquals(
      mat,
      Matrix.fromRows(
        NArray[Double](101.0, 202.0, 303.0, 404.0),
        NArray[Double](105.0, 206.0, 307.0, 408.0),
        NArray[Double](109.0, 210.0, 311.0, 412.0),
        NArray[Double](113.0, 214.0, 315.0, 416.0),
        NArray[Double](113.0, 214.0, 315.0, 416.0)
      )
    )
  ...
  test("SIMD-optimized array addition to columns (dense row-major)"):
    ...
    val r2 = NArray[Double](100.0, 200.0, 300.0, 400.0, 500)
...
// Removed unnecessary debug print statements
    assertMatrixEquals(
      transposeCheck,
      Matrix.fromRows(
        NArray[Double](101.0, 205.0, 309.0, 413.0, 513.0),
        NArray[Double](102.0, 206.0, 310.0, 414.0, 514.0),
        NArray[Double](103.0, 207.0, 311.0, 415.0, 515.0),
