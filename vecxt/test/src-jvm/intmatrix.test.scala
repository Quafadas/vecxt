...
class IntMatrixJvmSuite extends FunSuite:
  ...
  test("sum reduction, genuinely non-contiguous (padded strides)"):
    ...
    val mat = Matrix[Int](Array[Int](1, 2, 99, 3, 4, 99), 2, 2, 1, 3, 0)
    assert(!mat.hasSimpleContiguousMemoryLayout)

    assertIntMatrixEquals(mat.sum(0), Matrix(Array[Int](4, 6), (2, 1)))
    assertIntMatrixEquals(mat.sum(1), Matrix(Array[Int](3, 7), (1, 2)))
